          ...
          <div class="grid-content">
            ...
            <el-col :span="8">
              <div class="grid-content-sub-heading grid-content-sub-heading-bg-center" @dblclick="MakeRowEditable()" style="overflow-x: scroll;">
                <div v-if="(!Edit)">{{model.tradeGroupName}}</div>
                <el-select v-model="model.tradeGroupId" placeholder="Please Select" size="mini" v-if="(Edit)">
                  <el-option
                    v-for="(item, index) in getTradeGroupsDisplayData"
                    :key="index"
                    :label="item.description"
                    :value="item.tradeGroupId"
                  ></el-option>
                </el-select>
              </div>
            </el-col>
              ...
              <el-col :span="8" style="overflow-x: scroll; background: #34495E;">
                <div class="grid-content-buttons grid-content-bg" style="width 90%;" >
                  ...
                  <el-tooltip :enterable="false" effect="dark" v-if="(Edit)" placement="top" content="Save Changes" >
                    <el-button v-if="(Edit)" size="mini" type="success" icon="fas fa-save" circle @click="UpdateTrade(trades)"></el-button>
                  </el-tooltip>
                  <el-tooltip :enterable="false" effect="dark" placement="top" content="Delete Trade" v-if="(model.deleted === false)">
                    <el-button v-if="(model.deleted === false)" size="mini" type="danger" icon="el-icon-delete" circle @click="removeTrade(trades, index)" ></el-button>
                  </el-tooltip>
                  <el-tooltip :enterable="false" effect="dark" placement="top" content="Reactivate Trade" v-if="model.deleted === true">
                    <el-button v-if="model.deleted === true" size="mini" type="success" icon="fas fa-trash-restore" circle @click="reactivateTrade(trades, index)" ></el-button>
                  </el-tooltip>
                </div>
              </el-col>
            </el-row>
...
export default {
  ...
  methods: {
    ...
    UpdateTrade (item) {
      this.CreateTrade(item)
    },
    CreateTrade (item) {
      this.trades.tradeGroupId = this.model.tradeGroupId
      this.trades.description = this.model.description
      if ((this.trades.tradeGroupId !== null) && (this.trades.tradeGroupId !== undefined) && (this.trades.description !== null) && (this.trades.description !== undefined) && (/\S/.test(this.trades.description))) {
        this.setGlobalLoading(true)
        this.$store.dispatch({
          type: 'addorUpdateTradeDataAndReload',
          data: item
        }).then(() => {
          this.showSuccessNotification('Trade was successfully Added.')
        }).catch((error) => {
          console.log(error)
        }).finally(() => {
          this.Edit = false
        ...
        this.showWarningNotification('Missing Information, Please ensure u provide a Trade name and Description')
        this.trades.tradeGroupId = null
        this.trades.description = null
      }
    },
    ...
    removeTrade (item, index) {
      this.$confirm('Do you want to disable this?', 'Warning', {
        ...
        this.setGlobalLoading(true)
        this.$store.dispatch({
          type: 'DisableOrEnableTradeDataAndReload',
          Id: item.id
        }).then(() => {
    ...
    reactivateTrade (item, index) {
      this.$confirm('Do you want to reactivate this?', 'Warning', {
        ...
        this.$store.dispatch({
          ...
          this.showSuccessNotification('Trade was successfully reactivated.')
        }).catch((error) => {
          console.log(error)
        }).finally(() => {
          this.setGlobalLoading(false)
  ...
  computed: {
    getTradeGroupsDisplayData () {
      return this.$store.getters.getTradeGroupsDisplay
    }
  }
}
</script>
